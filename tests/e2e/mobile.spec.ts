...
test('feature cards visible on mobile', async ({ page }) => {
  await page.goto('/');
  const cards = page.locator('#how-mellea-section').getByRole('article');
  const count = await cards.count();
  expect(count).toBeGreaterThanOrEqual(4);
  await expect(cards.first()).toBeVisible();
});

test('future panel tabs visible on mobile', async ({ page }) => {
  await page.goto('/');
...
test('GitHub button visible on mobile', async ({ page }) => {
  await page.goto('/');
